...
AllocStrategy alloc_mode = CARTALLOC_RANDOM;		//Memory allocation strategies

FileAllocationTable *file_alloc_table;		//Table for files

int num_of_file;		//Number of files in the driver

int current_cart;		//The current cartridge this driver working on
...
int address_occupied(FileAddress file_address) {
	...
	for(int i = 0; i < num_of_file; i++) {
		for (int j = 0; j < file_alloc_table[i].num_of_address; j++){

			if(file_alloc_table[i].file_address[j].cartridge == file_address.cartridge
				&& file_alloc_table[i].file_address[j].frame == file_address.frame) {
				return(1);
			}
		}
	}
...
int load_cart(int cart_num) {
	...
	if (current_cart != cart_num) {
		if (extract_cart_opcode(cart_io_bus(creat_cart_opcode(CART_OP_LDCART, 0, cart_num, 0), NULL)) == 1){
...
int32_t cart_poweron(void) {
	...
	if (extract_cart_opcode(cart_io_bus(creat_cart_opcode(CART_OP_INITMS, 0, 0, 0), NULL)) == 1) {
		return(-1);
	}
	...
	for (int i = 0; i < CART_MAX_CARTRIDGES; i++) {
		...
		if (extract_cart_opcode(cart_io_bus(creat_cart_opcode(CART_OP_BZERO, 0, 0, 0), NULL)) == 1){
...
int32_t cart_poweroff(void) {
	...
	if (extract_cart_opcode(cart_io_bus(creat_cart_opcode(CART_OP_POWOFF,0,0,0), NULL)) == 1) {
...
int32_t cart_read(int16_t fd, void *buf, int32_t count) {
	...
	if (count <= CART_FRAME_SIZE - offset) {
		...
		//read frame
		if (extract_cart_opcode(cart_io_bus(creat_cart_opcode(CART_OP_RDFRME,0,0, frame), temp)) == 1) {
...
int32_t cart_write(int16_t fd, void *buf, int32_t count) {
	...
	if (address_index == file_alloc_table[file_index].num_of_address) {
	...
	if (count <= CART_FRAME_SIZE - offset) {
		...
		//write to frame
		if (extract_cart_opcode(cart_io_bus(creat_cart_opcode(CART_OP_WRFRME,0, 0, frame), temp)) == 1) {
		...
		for (int i = 0; i < num_of_frame; i++) {
			...
			//check if new memory needed
			if (address_index + i == file_alloc_table[file_index].num_of_address) {
				//allocate new address
				grow_file_address_list(&file_alloc_table[file_index]);
			}
