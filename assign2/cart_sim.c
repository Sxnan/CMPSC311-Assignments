...
int main( int argc, char *argv[] ) {
	...
	AllocStrategy alloc_mode = CARTALLOC_RANDOM;	//Defaults to random mode
	int modeset = 0;	//flag if the mode has been set

	struct option long_options[] = 
	{
		{"linear", no_argument, (int *)&alloc_mode, 1},
		{"balanced", no_argument, (int *)&alloc_mode, 2},
		{"random", no_argument, (int *)&alloc_mode, 0},
		{0,0,0,0}
	};

	// Process the command line parameters
	while ((ch = getopt_long(argc, argv, CART_ARGUMENTS, long_options, NULL)) != -1) {
		switch (ch) {
		case 0: //long_option flag set
			 //check if the mode has been set
			 if (modeset == 0) {
				modeset = 1;
				break;
			 } else {
			 	printf("More than one alloction mode provided, aobrting.\n");
				return (-1);
			 }
	...
	// Set the driver's allocation mode
	cart_setMode(alloc_mode);
